...
public class ExportProcess extends SvrProcess {
	...
	protected String getFileExtension() {
		if (getExportFormat().getExtension() != null) {
			if (this.getExportFormat().getExtension().equals(X_AD_ExpFormat.EXTENSION_CSV))
				return FILE_EXTENSION_CSV;
			else if (this.getExportFormat().getExtension().equals(X_AD_ExpFormat.EXTENSION_TXT))
	...
	public String getRowSeparator() {
		if (this.getExportFormat().getEndLineType() != null)
			if (this.getExportFormat().getEndLineType().equals(X_AD_ExpFormat.ENDLINETYPE_Unix))
				return ROW_SEPARATOR_UNIX;
			else if (this.getExportFormat().getEndLineType().equals(X_AD_ExpFormat.ENDLINETYPE_Windows))
				return ROW_SEPARATOR_WINDOWS;
		return ROW_SEPARATOR_UNIX;
	}
	...
	public String getEncodingType() {
		if (this.getExportFormat().getEncodingType() != null) {
			if (this.getExportFormat().getEncodingType().equals(X_AD_ExpFormat.ENCODINGTYPE_8859_1))
				return ENCODE_8859_1;
			else if (this.getExportFormat().getEndLineType().equals(X_AD_ExpFormat.ENCODINGTYPE_UTF_8))
				return ENCODE_UTF8;
		}
		return ENCODE_UTF8;
	}
	...
	protected String getQuery() {
		...
		// FROM
		sql.append(" FROM ");
		sql.append(M_Table.getTableName(getCtx(), getExportFormat().getAD_Table_ID()));
		// WHERE
		sql.append(" WHERE ");
	...
	protected String getWhereClause() {
		...
		whereClause.append(" ad_client_id = ").append(Env.getAD_Client_ID(getCtx()));
	...
	protected String applyColumnFormat(MExpFormatRow expFormatRow, String data) {
		...
		String newData = data;
		if (MExpFormat.FORMATTYPE_FixedPosition.equals(getExportFormat().getFormatType()) 
				&& expFormatRow.getLength() > 0) {
	...
	private void loadFieldSeparator() {
		String separator = "";
		if (MExpFormat.FORMATTYPE_TabSeparated.equals(getExportFormat().getFormatType())) {
			separator = "	";
	...
	protected String getMsg() {
		return Msg.getMsg(getCtx(), "ExportedRecords") + ": " + getExportedLines();
	}
